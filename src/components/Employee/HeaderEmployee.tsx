...
import { BriefcaseBusiness, KeyRound, LogOut, X } from "lucide-react";
...
const HeaderEmployee = (): JSX.Element => {
    ...
    const [changeStateLoading, setChangeStateLoading] = useState<boolean>(false);
    const [lastestShift, setLastestShift] = useState<number>(-1);
    const [showChange, setShowChange] = useState<boolean>(false);

    const itemMenu: {label: string, path: string, icon: any}[] = [
        {label: "Ca làm", path: "/employee/home", icon: BriefcaseBusiness},
        {label: "Đổi mật khẩu", path: "/employee/home", icon: KeyRound},
    ...
    const getState = async (): Promise<void> => {
        try {
            const result: BackendResponse = await employeeService.getStateApi();
            if (result.code == 0) {
                if (result.data < 0) {
                    setState(false);
                } else {
                    setState(true);
                    setLastestShift(result.data);
                }
            } else {
            ...
            console.log(e);
            messageService.error("Xảy ra lỗi ở servier");
        }
    }

    const changeShiftState = async (): Promise<void> => {
        setChangeStateLoading(true);
        try {
            const now = dayjs().toISOString();
            const result = await employeeService.changeShiftStateApi(now, state, lastestShift);
            if (result.code == 0) {
                setState(!state);
                setLastestShift(result.data);
    ...
    return(
        <>
            <Row className="header-employee-container">
                ...
                <Col span={width < 800 ? 12 : 24} style={{display: "flex", justifyContent: "end", alignItems: "center", paddingRight: "30px"}}>
                    <Button
                        size="large"
                        variant="solid"
                        color="primary"
                        onClick={() => {changeShiftState()}}
                    >
                        {`${!state ? "Vào ca" : "Ra ca"}`}
                    </Button>
                </Col>
            ...
            <Row className="menu-outlet-container">
                ...
                <Col span={width < 800 ? 24 : 18} style={{display: "flex", justifyContent: "center", alignItems: "center"}}>
                    <div className="outlet page-container">{<Outlet context={{state}} />}</div>
                </Col>
            </Row>
            <AnimatePresence mode="wait">
                {
